...
import "../../contracts/KingOFTUpgradeable.sol";
...
struct OFTDeployment {
    ...
    KingOFTUpgradeable tokenContract;
}
...
contract DeployOFTScript is Script, L2Constants {
    ...
    function deployOFT() internal {
        ...
        oftDeployment.implementationAddress = address(new KingOFTUpgradeable{salt: SALT}(DEPLOYMENT_LZ_ENDPOINT));
        oftDeployment.proxyAddress = address(
            new TransparentUpgradeableProxy{salt: SALT}(
                oftDeployment.implementationAddress,
                scriptDeployer,
                abi.encodeWithSelector(
                    KingOFTUpgradeable.initialize.selector, TOKEN_NAME, TOKEN_SYMBOL, scriptDeployer
                )
            )
        );

        oftDeployment.tokenContract = KingOFTUpgradeable(oftDeployment.proxyAddress);
