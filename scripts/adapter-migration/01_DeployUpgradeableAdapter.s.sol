...
import "../../contracts/KingOFTAdapterUpgradeable.sol";
...
contract DeployUpgradeableOFTAdapter is Script, L2Constants {
    ...
    function run() public returns (address) {
        ...
        address adapterImpl = address(new KingOFTAdapterUpgradeable(L1_WEETH, L1_ENDPOINT));
        KingOFTAdapterUpgradeable adapter = KingOFTAdapterUpgradeable(address(
            new TransparentUpgradeableProxy(
                address(adapterImpl),
                L1_TIMELOCK,
                abi.encodeWithSelector( // delegate and owner stay with the deployer for now
                    KingOFTAdapterUpgradeable.initialize.selector, scriptDeployer, scriptDeployer
                )
            ))
        );
